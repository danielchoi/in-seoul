# AI SDK 6 Beta + OpenAI Responses API
...
The Responses API includes a built-in `fileSearch` tool for searching documents in OpenAI vector stores. This enables RAG (Retrieval-Augmented Generation) workflows.

**Prerequisites:**
1. Create a vector store at [OpenAI Platform](https://platform.openai.com/storage/vector_stores)
2. Upload files to the vector store
3. Store the vector store ID in your environment
...
const result = await generateText({
  ...
  tools: {
    // Tool MUST be named 'file_search' - OpenAI API requirement
...
const result = await generateText({
  model: openai.responses('gpt-4o'),
  prompt: 'Find marketing documents from 2024',
  tools: {
    file_search: openai.tools.fileSearch({
      vectorStoreIds: ['vs_xxx'],
      filters: {
        type: 'and',
        filters: [
          { type: 'eq', key: 'category', value: 'marketing' },
          { type: 'gte', key: 'date', value: 1704067200 }, // Jan 1, 2024
        ],
      },
    }),
  },
...
1. **Response Methods**:
   - `toDataStreamResponse()` → `toUIMessageStreamResponse()` (for useChat)
   - `toTextStreamResponse()` available for simple text streaming
...
2. **Message Conversion**:
   ...
   const result = streamText({
     model: openai.responses('gpt-4o'),
     messages: convertToModelMessages(messages), // Required for useChat
   });
   ```

3. **Tool Factories**:
   ```ts
   // webSearchPreview now requires config object (can be empty)
   openai.tools.webSearchPreview({})

   // fileSearch unchanged
   openai.tools.fileSearch({ vectorStoreIds: [...] })
   ```

### New Features in AI SDK 6

- **Agent Abstraction**: Unified interface for building agents with `ToolLoopAgent`
- **Tool Execution Approval**: Human-in-the-loop workflows
- **Structured Output**: Stable support for generating structured data with tools
- **Reranking**: Document reordering with specialized models (Cohere, Bedrock, Together.ai)
- **Call Options**: Type-safe runtime configuration for dynamic behavior

## Get Started

Ready to get started? Here's how you can dive in:

1. Explore the documentation at [v6.ai-sdk.dev/docs](https://v6.ai-sdk.dev/docs) for AI SDK 6 beta.
2. Check out practical examples at [ai-sdk.dev/examples](/examples) to see the SDK in action.
3. Dive deeper with advanced guides on RAG and multi-modal chat at [ai-sdk.dev/docs/guides](/docs/guides).
